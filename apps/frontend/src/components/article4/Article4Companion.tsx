...
export function Article4Companion({ article }: { article: SeriesArticle }) {
  ...
  function navigate(id: LessonId, level: Level = query.level, nextMode: Mode = "watch") {
  ...
  return <div className="article4">
    <header className="a4-hero"><div className="a4-container">
      ...
      ].map(item => <a key={item.id} href={article4LessonPath(item.id, 100)} className="a4-entry" onClick={event => { event.preventDefault(); navigate(item.id, 100); }}><item.icon aria-hidden="true" /><span><strong>{item.title}</strong><small>{item.hint} →</small></span></a>)}</div>
    ...
    <div className="a4-container">
      ...
      <div className="a4-workspace">
        ...
        <div className="a4-main">
          ...
          <div className="a4-next"><span>Keep exploring</span><a href={article4LessonPath(lesson.nextLessonId, query.level)} onClick={event => { event.preventDefault(); navigate(lesson.nextLessonId); }}>Next: {ARTICLE4_LESSONS.find(item => item.id === lesson.nextLessonId)!.title}<ArrowRight size={15} aria-hidden="true" /></a></div>
